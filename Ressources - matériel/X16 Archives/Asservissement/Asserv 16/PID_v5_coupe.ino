...
void setup(){
  ...
  Krotation = 0.88;
  speedLanceur = 53  - int(1.7*deltaV); // reglage de base : 51
  distMilieu = 6;
  distGauche = 6;
  distDroite = 6;
  ...
  if (homologation){
    ...
    if (couleur()){
      parcoursHomologationGauche();
    }
    else{
      parcoursHomologationDroite();
    }
  }
...
void parcoursDroit(){
  ...
  aller(240);
  tourner(-57); // on se tourne vers la cible
  envoyerBalles();
  delay(2000);
  tourner(59);
  aller(190);
  tourner(-86);// vers l'abeille
  ...
  aller(700); //
  tourner(-87); // virage à droite vers la zone de calage
...
void parcoursGauche(){
  ...
  aller(700); //
  tourner(93); // virage à droite vers la zone de calage
