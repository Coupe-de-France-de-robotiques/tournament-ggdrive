...
void setup()
{
  ...
  cpr = 64; // number of counts per revolution of the encoder
...
void setParameters(long dist, long ang)
{
  ...
  angleTargetTemp = angleTargetTemp * trackWidth * cpr / (180 * wheelDiameter); // en incréments
...
void AsservSoft() // adapte la consigne au fur et à mesure
{
  ...
  if (erreurAngle < 5 && erreurAngle>-5 &&( (angleTarget>0 && temps*coefVitesseR>=angleTarget) || (angleTarget<=0 && -temps*coefVitesseR<=angleTarget))){
    compteurZeroAngle++; // dit depuis combien de temps on est au bon angle (en gros)
  }
